<!doctype html>
<html>
<head>
<meta charset="utf-8">
<title>Manager login</title>
    <link rel="stylesheet" href="css/bootstrap.min.css">
    <link rel="stylesheet" href="css/bootstrap-theme.min.css">
    <link rel="stylesheet" href="css/css.css">
</head>

<body>
	<img src="css/hrm_image.jpg" class="img-responsive" >
    <div class="container" style="background:#B5E61D; height:50px; border-top-style:solid;">
    </div>
    <div class="container">
    	<div class="col-sm-3"></div>
        <div class="col-sm-6 tabs" >
        	<ul class="nav nav-pills">
            	<li><a href="index.php">Admin login</a></li>
            	<li><a href="login/manager_login.php">Manager login</a></li>
            	<li class="active"><a href="">View section</a></li>
            </ul>
        </div>
        <div class="col-sm-3"></div>
    </div>
	<div class="container" >
    	<div class="col-sm-3">
        </div>
    	<div class="col-sm-6 login">
        	<h3> Select your category </h3>
            <ul class="nav nav-tabs nav-stacked">
            	<li><a href="login/employee_login.php">Employee information</a></li>
            	<li><a href="notice_board.php">Notice board</a></li>
            </ul>
        </div>
        <div class="col-sm-3">
        </div>
    </div>
    <script src="js/bootstrap.min.js"></script>
    <script src="js/jQuery.js"></script>
</body>
</html>
